...
依赖数据改变或使用`$forceUpdate`强制刷新 -> `beforeUpdate` -> 移除已经添加的事件监听器 -> `render -> updated` 操作DOM添加事件监听器
...
* 普通插槽：
  ...
  ```vue
  <template v-slot:slotname="xxx">...</template>
  ```
...
在Vue组件中，根实例可以通过`$root`属性进行访问，所有属性data、计算属性computed、方法methods均挂载到根实例上。但尽量使用Vuex来管理应用状态。 在子组件中可以使用`$parent`来访问父组件的实例。在父组件中也可以使用$refs访问子组件实例，但不是响应式的，可以通过调用methods里的方法来一次性获取。  
...
* `$route.params` 路由路径参数的`key/value`对象，对于模式为`/user/:username`，该对象为`{ username: 'username' }`，`$route.params.username === 'username'`
* `$route.query URL`路径参数的key/value对象，例如`/user?page=1`，该对象为`{ page : 1 }`，`$route.query.page === 1`
...
* State - `this.$store.state.xxx`取值 - 提供响应式数据
* Getter - `this.$store.getters.xxx`取值 - 借助computed来实现缓存
* Mutation - `this.$store.commit("xxx")`赋值 - 更改state方法
* Action - `this.$store.dispatch("xxx")`赋值 - 触发mutation方法
* Module - 模块化上方几个 - Vue.set动态添加state到响应式数据中
...
// 创建一个store，state是Store存着的共享的数据，mutations在更改数据时必须使用
const store = new Vuex.Store({
  state: {
    count: 0
  },
  mutations: {	// mutation必须同步执行，需要异步则使用action使用commit方式调用mutation
    increment (state, n) {
      state.count+=n;
    }
  },
  action: {	// action可以执行异步操作
      // 这里可以传context上下文，也可以使用{ commit, state }直接传递函数
      increment(context, n) {
          setTimeout(()=>{
...
<template>
	<div>
        ...
        <!-- 调用mutations内的一些功能 -->
        <button @click="$store.dispatch('increment', 2)">count=+2</button>
    </div>
</template>
<script>
export default {
...
组件读取State最方便的方式就是计算属性，Store内的`count`每次变化时都会更新该组件的`count`，但是这样会需要频繁的导入，在测试组件时也需要模拟状态
...
为了解决上述问题，可以在Vue实例化时通过store选项将Vuex根组件注册进去，并在组件中在computed中定义一下拿出来用。
...
```
如果将计算属性和state子节点名称相同时则可以直接给mapState传一个字符串数组

```js
computed: mapState([
    // 映射this.count（组件的count）为store.state.count
    'count'
])
```
...
有些时候我们需要从Store中的state内派生出一些状态，比如从Store的TodoList中派生出所有未做完的东西，这时我们在Store中定义一个Getter来实现这一点，这样定义的属性会被缓存起来并根据依赖值的变化而变化
...
可以使用方法调用的形式来访问，当在查询store里数组的时候非常有用
...
更改Store里的状态的唯一方式就是提交`Mutation`，`Action`需要通过提交`Mutation`来更改状态，`Mutation`只能做同步动作不能做异步动作，异步动作应该给`Action`做，做完了再`commit`调用`Mutation`
...
`Action`是一种对Store中的状态进行修改但必须通过调用`Mutation`实现，`Action`可以包含任意异步操作
...
const store = new Vuex.Store({
  ...
  actions: {
    increment (context) {
      context.commit('increment')
      // 在这里也可以使用state和getters
    }
  }
})
```

调用Action通过`store.dispatch`方式触发，同时也和`Mutation`差不多支持载荷
...
我们可以组合多个Action来进行链式操作，但首先我们要使用Promise
...
默认获得的状态、action都是局部的，如果向获得根节点的可以用rootState参数获取



### 处理表单

可以使用双向绑定的计算属性解决表单没法直接绑定Store的state来形成修改
...
// jest.config.js
module.exports = {
  moduleFileExtensions: ["js", "jsx", "json", "vue"],	// 文件类型
  transform: {		// 和webpack的loader差不多，指定处理对应后缀文件的处理器
    "^.+\\.vue$": "vue-jest",
    ".+\\.(css|styl|less|sass|scss|svg|png|jpg|ttf|woff|woff2)$":
      "jest-transform-stub",
    "^.+\\.jsx?$": "babel-jest"
  },
  transformIgnorePatterns: ["/node_modules/"],	// 忽略node_modules
  moduleNameMapper: {
    "^@/(.*)$": "<rootDir>/src/$1"	// 定义@别名直接访问src下的文件
  },
  snapshotSerializers: ["jest-serializer-vue"],	// 快照功能
  testMatch: [
    "**/tests/unit/**/*.spec.(js|jsx|ts|tsx)|**/__tests__/*.(js|jsx|ts|tsx)"	// 哪些文件要跑单元测试
  ],
  testURL: "http://localhost/"	// js DOM地址
};
```
...
describe("HelloWorld.vue", () => {
  const wrapper = mount(HelloWorld, {	// 挂载组件
      propsData: { msg },	// 传递props
      listeners: {	// 定义监听器，监听this.$emit('事件名', 值)发出的事件这里事件为change
          change() {
              isCalled = true;
          }
      }
  });
  it("renders props.msg when passed", () => {	// it是一个单元测试，render blabla是描述
    // 校验
    const msg = "new message";
    // 希望一样，这里使用toMatchSnapshot可以生成html快照，快速比对之的情况
...
* 项目中使用`Vue Router`会在项目`src`目录下新建一个`router/index.js`用于`router`代码，项目`main.js`内`import`该文件并在`new Vue()`的时候挂载到`Vue`上
...
* `NProgress`结合`Router`可以实现使用`Router`切换页面时上方有加载进度条
